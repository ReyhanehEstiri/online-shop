...
function Basket() {
  ...
  const removeItem = async (bookId, userId) => {
    try {
      const response = await fetch('http://localhost:7266/api/v1/Book/RemoveBookFromCart', {
        method: 'DELETE',
        headers: {
          'accept': '*/*',
  ...
  const handlePurchase = async () => {
    try {
      const response = await fetch('http://localhost:7266/api/v1/Book/PurchaseUserCart', {
        method: 'POST',
        headers: {
          'accept': '*/*',
        },
