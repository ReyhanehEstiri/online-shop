...
function Basket() {
  ...
  const removeItem = async (bookId, userId) => {
    try {
      const response = await fetch(' https://25ed-5-202-181-171.ngrok-free.app/api/v1/Book/RemoveBookFromCart', {
        method: 'DELETE',
        headers: {
          'accept': '*/*',
  ...
  const handlePurchase = async () => {
    try {
      const response = await fetch(' https://25ed-5-202-181-171.ngrok-free.app/api/v1/Book/PurchaseUserCart', {
        method: 'POST',
        headers: {
          'accept': '*/*',
        },
